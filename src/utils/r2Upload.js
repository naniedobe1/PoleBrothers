...
export const uploadToR2 = async (imageUri, filename) => {
  try {
    ...
    Logger.log('Requesting presigned URL from worker...');
    Logger.log('Worker URL:', CLOUDFLARE_WORKER_URL);
    Logger.log('Worker URL type:', typeof CLOUDFLARE_WORKER_URL);
    ...
    Logger.log('Upload URL:', uploadUrl.substring(0, 100) + '...');
    ...
    Logger.log('Reading image file from:', filePath);
    ...
    Logger.log('File read successfully, base64 length:', fileBase64.length);

    // Step 3: Upload to R2 using fetch with proper binary handling
    Logger.log('Starting R2 upload with fetch...');
    ...
    Logger.log('Binary data size:', bytes.length, 'bytes');
    ...
    Logger.log('R2 upload response status:', uploadResponse.status);

    if (!uploadResponse.ok) {
      const errorText = await uploadResponse.text();
      Logger.log('R2 upload error:', errorText);
